/**@file   gnn_bounds_nodeclassify.c
 * @brief  functions to compute bounds on variables and expressions in GNNs for node classification problems
   ...
   )
{
   ...
   for( f = 0; f < ninputfeatures; ++f )
   {
      ...
      for( v = 0; v < ngraphnodes; ++v )
      {
         /* skip non-existing edges, but not self-loops */
         if( !adjacencymatrix[v][sagegraphnodeidx] )
            continue;
   ...
   {
      /* compute index of arc entering the sagegraphnodeidx */
      ...
      /* we only proceed if existing edge is potentially attacked (arcs can only be removed) */
      ...
      for( f = 0; f < ninputfeatures; ++f )
      {
         ...
         if( ub < 0.5 && adjacencymatrix[v][sagegraphnodeidx] )
         {
   ...
   for( v = 0; v < ngraphnodes; ++v )
   {
      for( f = 0; f < nfeatures; ++f )
      {
         bound = computeBoundGNNSageLayerNode(layerinfo, TRUE, adjacencymatrix, ngraphnodes,
            adjacencyvars, MIN(globalbudget, localbudget), v, f,
   ...
   for( v = 0, i = 0; v < ngraphnodes; ++v )
   {
      for( w = 0; w < ngraphnodes; ++w )
      {
         for( f = 0; f < nfeatures; ++f )
         {
            ...
            /* compute index of arc entering v */
            edgeidx = getEdgeIdx(w, v, ngraphnodes);

            if( adjacencyvars != NULL )
            {
               lb = SCIPvarGetLbLocal(adjacencyvars[edgeidx]);
               ub = SCIPvarGetUbLocal(adjacencyvars[edgeidx]);
            }

            /* arcs can only be removed, so the auxiliary variable is 0 if the arc is not in the original graph */
   ...
   )
{
   ...
   case GNN_LAYERTYPE_INPUT:
      ...
      SCIP_CALL( computeBoundsGNNInputLayer(scip, inputinfo, ngraphnodes, globalbudget, localbudget,
            ...
            ubgnnoutputvars, ubauxvars, ubnodecontent, adjacencymatrix) );
      break;
   case GNN_LAYERTYPE_SAGE:
      ...
      SCIP_CALL( computeBoundsGNNSageLayer(scip, sageinfo, adjacencymatrix, ngraphnodes, globalbudget, localbudget,
            ...
            ubauxvars, ubnodecontent, lbgnnoutputvarsprev, ubgnnoutputvarsprev) );
      break;
   ...
   )
{
   ...
   if( gnnoutputvars != NULL )
      ...
      gnnoutvars = NULL;
   SCIP_CALL( SCIPcomputeBoundsGNNNodeClassifyLayer(scip, gnndata, adjacencymatrix, ngraphnodes,
