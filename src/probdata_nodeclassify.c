...
{
   /* global information about GNN and node classification problem */
   ...
   int                   localbudget;        /**< local attack budget on nodes */
...
SCIP_RETCODE createAdjacencyVariable(
   ...
   SCIP_Bool**           adjacencymatrix     /**< adjacency matrix */
   )
{
   ...
   /* fix arcs that do not exist to 0 (we can only delete arcs) */
   if( !adjacencymatrix[u][v] )
      ub = 0.0;

   SCIP_CALL( SCIPcreateVar(scip, var, name, 0.0, ub, 0.0, SCIP_VARTYPE_BINARY,
   ...
   )
{
   ...
   /* collect coefficients for variables leaving or entering the node */
   for( i = 0, cnt = 0; i < nnodes; ++i )
   {
      if( adjacencymatrix[nodeidx][i] )
      {
         vars[cnt] = adjacencyvars[getEdgeIdx(nodeidx, i, nnodes)];
         coefs[cnt++] = -1.0;
         rhs -= 1.0;
      }
      /* do not add self-loop twice */
      if( i != nodeidx && adjacencymatrix[i][nodeidx] )
      {
         vars[cnt] = adjacencyvars[getEdgeIdx(i, nodeidx, nnodes)];
         coefs[cnt++] = -1.0;
         rhs -= 1.0;
      }
      /* no positive coefficients since we are only allowed to delete arcs */
   }

   (void) SCIPsnprintf(name, SCIP_MAXSTRLEN, "localbudget%d", nodeidx);
   SCIP_CALL( SCIPcreateConsLinear(scip, cons, name, cnt, vars, coefs, -SCIPinfinity(scip), rhs,
         TRUE, TRUE, TRUE, TRUE, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE) );
   SCIP_CALL( SCIPaddCons(scip, *cons) );
   ...
   )
{
   ...
   (*targetprobdata)->localbudget = localbudget;
   ...
   )
{
   ...
   else
   {
      ...
      SCIP_CALL( SCIPallocBufferArray(scip, &vars, nadjvars) );
      ...
      for( i = 0, cnt = 0; i < nnodes; ++i )
      {
         for( j = 0; j < nnodes; ++j )
         {
            SCIP_CALL( createAdjacencyVariable(scip, &targetprobdata->adjacencyvars[cnt++], i, j, adjmat) );
         }
      }
      ...
      for( i = 0; i < nnodes; ++i )
      {
         SCIP_CALL( createLocalAttackBudgetCons(scip, &targetprobdata->localattackconss[i], nnodes, i,
               adjmat, targetprobdata->adjacencyvars, vars, coefs, targetprobdata->localbudget) );
      }
   ...
   for( v = 0, cnt = 0; v < nnodes; ++v )
   {
      for( w = 0; w < nnodes; ++w )
      {
         for( f = 0; f < nfeatures; ++f )
         {
            ...
            /* if an arc entering node v does not exist, then the auxiliary variable can be fixed
             * to 0 as arcs can only be deleted
             */
            if( !adjacencymatrix[w][v] )
               ub = 0.0;

            SCIP_CALL( SCIPcreateVar(scip, &(*auxvars)[cnt], name, lbs[cnt], ub, 0.0,
                  SCIP_VARTYPE_CONTINUOUS, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE) );
   ...
   for( v = 0, cnt = 0; v < nnodes; ++v )
   {
      for( w = 0; w < nnodes; ++w )
      {
         for( f = 0; f < nfeatures; ++f )
         {
            ...
            /* we only consider arcs entering v as neighbors relevant for auxiliary variables */
            edgeidx = getEdgeIdx(w, v, nnodes);
   ...
   )
{
   ...
   assert(auxvarsprev != NULL || typeprev == GNN_LAYERTYPE_POOL || typeprev == GNN_LAYERTYPE_DENSE || lidx == 1);
   ...
   storagelen = (nodebound + 1) * nfeaturesprev + 2;
            ...
            for( w = 0; w < nnodes; ++w )
            {
               for( f2 = 0; f2 < nfeaturesprev; ++f2 )
               {
                  ...
                  if( lidx == 1 )
                  {
                     ...
                     gnnidx = w*nfeaturesprev + f2;
                     assert(SCIPisEQ(scip, SCIPvarGetLbLocal(gnnoutputvarsprev[gnnidx]),
                           SCIPvarGetUbLocal(gnnoutputvarsprev[gnnidx])));

                     vars[cnt] = adjacencyvars[getEdgeIdx(w, v, nnodes)];
                     c = -SCIPgetSageLayerFeatureEdgeweight(sageinfo, f2, f);
                     c = c * SCIPvarGetLbLocal(gnnoutputvarsprev[gnnidx]);
                     coefs[cnt++] = c;
                  }
                  else
                  {
                  ...
                  /* we also need to take the node weights into account */
                  if( v == w && lidx != 1 )
                  {
                     gnnidx = w*nfeaturesprev + f2;
                     vars[cnt] = gnnoutputvarsprev[gnnidx];
                     coefs[cnt++] = -SCIPgetSageLayerFeatureNodeweight(sageinfo, f2, f);
                  }
                  else if( v == w )
                  {
                     gnnidx = w*nfeaturesprev + f2;
                     assert(SCIPisEQ(scip, SCIPvarGetLbLocal(gnnoutputvarsprev[gnnidx]), SCIPvarGetUbLocal(gnnoutputvarsprev[gnnidx])));
                     bias += SCIPgetSageLayerFeatureNodeweight(sageinfo, f2, f) * SCIPvarGetLbLocal(gnnoutputvarsprev[gnnidx]);
                  }
                  assert(cnt <= storagelen);
               }
            }
         }
         else
   ...
   )
{
   ...
   /* allocate temporary memory for variables and coefficients */
   storagelen = (nodebound + 1) * nsourcefeatures + 1;
   SCIP_CALL( SCIPallocBufferArray(scip, &vars, storagelen) );
   SCIP_CALL( SCIPallocBufferArray(scip, &coefs, storagelen) );
            ...
            for( w = 0; w < nnodes; ++w )
            {
               for( f2 = 0; f2 < nsourcefeatures; ++f2 )
               {
                  ...
                  if( targetlayeridx == 1 )
                  {
                     ...
                     gnnidx = w*nsourcefeatures + f2;
                     assert(SCIPisEQ(scip, SCIPvarGetLbLocal(sourcegnnoutputvars[gnnidx]),
                           SCIPvarGetUbLocal(sourcegnnoutputvars[gnnidx])));

                     vars[cnt] = adjacencyvars[getEdgeIdx(w, v, nnodes)];
                     c = -SCIPgetSageLayerFeatureEdgeweight(sageinfo, f2, f);
                     c = c * SCIPvarGetLbLocal(sourcegnnoutputvars[gnnidx]);
                     coefs[cnt++] = c;
                  }
                  else
                  {
                  ...
                  /* we also need to take the node weights into account */
                  if( v == w && targetlayeridx != 1 )
                  {
                     gnnidx = v*nsourcefeatures + f2;
                     vars[cnt] = sourcegnnoutputvars[gnnidx];
                     coefs[cnt++] = -SCIPgetSageLayerFeatureNodeweight(sageinfo, f2, f);
                  }
                  else if( v == w )
                  {
                     gnnidx = v*nsourcefeatures + f2;
                     assert(SCIPisEQ(scip, SCIPvarGetLbLocal(sourcegnnoutputvars[gnnidx]), SCIPvarGetUbLocal(sourcegnnoutputvars[gnnidx])));
                     bias += SCIPgetSageLayerFeatureNodeweight(sageinfo, f2, f) * SCIPvarGetLbLocal(sourcegnnoutputvars[gnnidx]);
                  }
                  assert(cnt <= storagelen);
               }
            }
   ...
   )
{
   ...
   else
   {
      ...
      SCIP_CALL( createPlainLinkingLayer(scip, layerlinkingconss, nnodes, targettype, targetactivation,
            sourcetype, sourceactivation, ntargetfeatures, nsourcefeatures, targetlayeridx,
            adjacencyvars, targetgnnoutputvars, sourceauxvars, sourcegnnoutputvars, gnndata) );
   }
      ...
      else
      {
         ...
         if( nauxvars > 0 && l > 0 )
         {
            SCIP_CALL( createAuxVarsLayer(scip, &targetprobdata->auxvars[l], type,
                  nnodes, nfeatures, l, adjacencymatrix, lbauxvars[l], ubauxvars[l]) );
         }
         else
            targetprobdata->auxvars[l] = NULL;
   ...
   )
{
   ...
   SCIP_CALL( probdataCreateLayer(scip, sourcedata, *targetdata, adjacencymatrix, featurelb, featureub,
...
SCIP_RETCODE setObjective(
   ...
   int                   nodeidx,            /**< index of node that shall be classified */
   int                   origclass,          /**< original classification of underlying graph */
   int                   targetclass         /**< target classification of underlying graph */
   )
{
   ...
   assert(SCIPgetGNNLayerType(gnndata, lidx) == GNN_LAYERTYPE_SAGE);

   sageinfo = SCIPgetGNNLayerinfoSage(gnndata, lidx);
   nfeatures = SCIPgetNOutputFeaturesSageLayer(sageinfo);
   ...
   SCIP_CALL( SCIPchgVarObj(scip, gnnoutputvars[lidx][nodeidx*nfeatures + origclass], 1.0) );
   SCIP_CALL( SCIPchgVarObj(scip, gnnoutputvars[lidx][nodeidx*nfeatures + targetclass], -1.0) );
   ...
   )
{
   ...
   /* transform variables */
   SCIP_CALL( SCIPtransformVars(scip, nnodes * nnodes, probdata->adjacencyvars, probdata->adjacencyvars) );
   for( l = 0; l < nlayers; ++l )
   {
      type = probdata->layertypes[l];
      activation = probdata->activation[l];
...
static
SCIP_DECL_PROBDELORIG(probdelorigNodeClassify)
{
...
static
SCIP_DECL_PROBDELTRANS(probdeltransNodeClassify)
{
...
static
SCIP_DECL_PROBTRANS(probtransNodeClassify)
{
...
{
   GNNPROB_NODECLASSIFY* rcprobdata;
   SCIP_PROBDATA* probdata;
   SCIP_Bool** adjacencymatrix;
   SCIP_Real** featurelb;
   SCIP_Real** featureub;
   int localbudget;
   int nodeclassification;
   ...
   rcprobdata = SCIPgetGNNProbDataNodeClassify(gnnprobdata);
   ...
   SCIP_CALL( SCIPsetProbDelorig(scip, probdelorigNodeClassify) );
   SCIP_CALL( SCIPsetProbTrans(scip, probtransNodeClassify) );
   SCIP_CALL( SCIPsetProbDeltrans(scip, probdeltransNodeClassify) );

   /* create basic problem data */
   nnodes = SCIPgetNNodesNodeClassify(rcprobdata);
   adjacencymatrix = SCIPgetAdjacencyMatrixNodeClassify(rcprobdata);
   globalbudget = SCIPgetGlobalBudgetNodeClassify(rcprobdata);
   localbudget = SCIPgetLocalBudgetNodeClassify(rcprobdata);
   nodeclassification = SCIPgetNodeclassNodeClassify(rcprobdata);
   graphclassification = SCIPgetGraphClassificationNodeClassify(rcprobdata);
   targetclassification = SCIPgetTargetClassificationNodeClassify(rcprobdata);

   featurelb = SCIPgetFeatureLbNodeClassify(rcprobdata);
   featureub = SCIPgetFeatureUbNodeClassify(rcprobdata);
   ...
   SCIP_CALL( setObjective(scip, gnndata, probdata->gnnoutputvars, nodeclassification,
         graphclassification, targetclassification) );
